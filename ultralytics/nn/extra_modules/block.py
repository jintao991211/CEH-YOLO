...
__all__ = ['DyHeadBlock', 'DyHeadBlockWithDCNV3', 'Fusion', 'C2f_Faster', 'C3_Faster', 'C3_ODConv', 'C2f_ODConv', 'Partial_conv3', 'C2f_Faster_EMA', 'C3_Faster_EMA', 'C2f_DBB',
           ...
           'BiFusion', 'RepBlock', 'C3_EMBC', 'C2f_EMBC', 'SPPF_LSKA','ESPPF', 'C3_DAttention', 'C2f_DAttention','C2f_DAttention_16', 'C3_Parc', 'C2f_Parc', 'C3_DWR', 'C2f_DWR',
...
class Bottleneck_DAttention(Bottleneck):
    ...
    def __init__(self, c1, c2, fmapsize, shortcut=True, g=1, k=(3, 3), e=0.5):  # ch_in, ch_out, shortcut, groups, kernels, expand
        super().__init__(c1, c2, shortcut, g, k, e)
        c_ = int(c2 * e)  # hidden channels
        self.attention = DAttention_16(c2, fmapsize)
    
    def forward(self, x):
        return x + self.attention(self.cv2(self.cv1(x))) if self.add else self.attention(self.cv2(self.cv1(x)))


class Bottleneck_DAttention_16(Bottleneck):
    """Standard bottleneck with DAttention."""
...
class C2f_DAttention_16(C2f):
    def __init__(self, c1, c2, n=1, fmapsize=None, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Bottleneck_DAttention_16(self.c, self.c, fmapsize, shortcut, g, k=(3, 3), e=1.0) for _ in range(n))
######################################## C3 C2f DCNV2_Dynamic end ########################################
...
#         super().__init__()
#         c_ = c1 // 2  # hidden channels
#         self.cv1 = EConv(c1, c_, 1, 1)
#         self.cv2 = EConv(c_ * 4, c2, 1, 1)
#         self.m = nn.MaxPool2d(kernel_size=k, stride=1, padding=k // 2)
#
#     def forward(self, x):
#         """Forward pass through Ghost Convolution block."""
