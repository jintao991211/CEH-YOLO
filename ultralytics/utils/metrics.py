...
#         w = [0.0, 0.0, 1.0, 0.0]  # weights for [P, R, mAP@0.5, mAP@0.5:0.95]改过的
...
def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, EIoU=False, SIoU=False, ShapeIoU=False,
             eps=1e-7, scale=0.0):
    """
    Calculate Intersection over Union (IoU) of box1(1, 4) to box2(n, 4).
...
class WiseIouLoss(torch.nn.Module):
    ...
    def forward(self, pred, target, ret_iou=False, ratio=1.0, **kwargs):
        self._fget = {
            ...
            # IoU / Inner-IoU
            'iou': (1 - get_inner_iou(pred, target, xywh=False,
                                      ratio=ratio).squeeze()) if self.inner_iou else lambda: 1 - self['s_inter'] / self[
                's_union'],
        }
